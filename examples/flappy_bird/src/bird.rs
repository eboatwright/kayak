...
pub fn bird_update(game_state: &mut GameState, context: &mut Context) {
    ...
    if game_state.bird.transform.y < -context.viewport.screen_size().y * 0.5 {
        game_state.bird.y_velocity = 0.0;
    }

    game_state.bird.transform.y = clamp(
        game_state.bird.transform.y,
        -context.viewport.screen_size().y * 0.5,
        context.viewport.screen_size().y * 0.5 - game_state.bird.transform.h,
    );
